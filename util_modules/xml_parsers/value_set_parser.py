...
class ValueSetParser(XMLParserBase):
    ...
    def parse_value_set(self, valueset_elem: ET.Element) -> Optional[Dict]:
        ...
        try:
            ...
            for values_elem in all_values_elems:
                value_data = self._parse_value_entry(values_elem)
                if value_data:
            ...
            if all_values_elem is not None:
                result['all_values'] = self._parse_all_values(all_values_elem)
    ...
    def _parse_value_entry(self, values_elem: ET.Element) -> Optional[Dict]:
    ...
    def _parse_all_values(self, all_values_elem: ET.Element) -> Dict:
        """Parse allValues structure (alternative to individual values)"""
        try:
            result = {
                'code_system': self.parse_child_text(all_values_elem, 'codeSystem'),
                'values': []
            }
