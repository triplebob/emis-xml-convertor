...
def display_error_to_user(error: EMISConverterError, 
    ...
    if show_technical_details and st.session_state.get('debug_mode', False):
...
def check_session_state_errors():
    ...
    if 'pending_errors' in st.session_state:
        errors = st.session_state.pending_errors
        for error in errors:
        ...
        del st.session_state.pending_errors
...
def store_error_for_display(error: EMISConverterError):
    ...
    st.session_state.pending_errors.append(error)
